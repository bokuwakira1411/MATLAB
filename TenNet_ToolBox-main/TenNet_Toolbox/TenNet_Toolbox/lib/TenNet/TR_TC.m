% FCTN_TC - Use Tensor Ring decomposition to complete the LRTC model.
%
% Syntax: [X, G] = TR_TC(F, Omega, varargin)
%
% Inputs:
% F - n_1*...*n_N tensor
% Omega - Index of the observed entries
% varargin - Optional variable parameter in Matlab. The fields are:
% varargin{1} - TR-rank: a vector of (N).
...
%
% Outputs:
% X - Recovery tensor based on Tensor Ring Decomposition.
% G - Factors of X based on Tensor Ring decomposition.
% The size of Gk is R_{k-1}*n_k*R_{k} when k ={2,..,N}.
% The size of G1 is R_N*n_1*R_{1}. 
%
% Example:
% [X, G] = TR_TC(F, Omega, R, rho, tol, maxit)
%
function [X, G] = TR_TC(F,Omega,varargin)
%% Default parameter
...
if length(varargin{1})>=1
   ...
   R(1,N) = R1(N); 
    ...
    error('Please enter a vector of N as tr-rank!');
end
